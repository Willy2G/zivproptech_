...
import { Calendar, Loader2, Search, Share2, Check } from 'lucide-react';
...
function CardShareButton({ title, slug }) {
  ...
  return (
    <button
      onClick={handleShare}
      className={`w-8 h-8 flex items-center justify-center rounded-full bg-white/90 backdrop-blur-sm shadow-sm transition-all duration-200 ${copied ? 'text-green-500' : 'text-gray-500 hover:text-ziv-cyan hover:bg-white'}`}
      title="Partager"
    >
                ...
                >
                  <div className="h-56 overflow-hidden relative">
                    ...
                    <div className="absolute top-4 right-4">
                      <CardShareButton title={post.title} slug={post.slug} />
                    </div>
                  </div>
